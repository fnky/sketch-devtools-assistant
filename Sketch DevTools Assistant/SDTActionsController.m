...
-(void)initializeCommandObservers {
  ...
  [center addObserver:self selector:@selector(externalCommandReleaseNormal:)
                 name:@"release-normal" object:observedObject];
  [center addObserver:self selector:@selector(externalCommandReleaseSandboxed:)
                 name:@"release-sandboxed" object:observedObject];
  [center addObserver:self selector:@selector(externalCommandBetaNormal:)
                 name:@"beta-normal" object:observedObject];
  [center addObserver:self selector:@selector(externalCommandBetaSandboxed:)
                 name:@"beta-sandboxed" object:observedObject];
}
...
-(void)externalCommandReleaseSandboxed:(NSNotification*)notification {
...
-(void)externalCommandBetaSandboxed:(NSNotification*)notification {
  [self processNotifications:true isBeta:true];
}

-(void)processNotifications:(BOOL)isSandboxed isBeta:(BOOL)isBeta {
  NSString* fileName = [(isSandboxed ?
                         (isBeta ? kPluginFolderPathBetaSandboxed :
                          kPluginFolderPathSandboxed) :
                         kPluginFolderPathNormal) stringByExpandingTildeInPath];
  NSString* filePath = [NSString stringWithFormat:@"%@/%@", fileName, kExchangeFileName];
  NSDictionary *commandInfo = [NSJSONSerialization JSONObjectWithData:[NSData dataWithContentsOfFile:filePath]
                                                              options:kNilOptions
                                                                error:nil];
...
+(id)sketchAppController:(NSString*)bundleID {
  NSString* bundleName = ([bundleID isEqualToString:@"com.bohemiancoding.sketch3.beta"]) ? @"Sketch Beta" : @"Sketch";
  return [[COScript app:bundleName] delegate];
}
